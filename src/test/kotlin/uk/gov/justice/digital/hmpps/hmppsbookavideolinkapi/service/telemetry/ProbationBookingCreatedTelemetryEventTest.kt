...
class ProbationBookingCreatedTelemetryEventTest {
  ...
  fun `should raise a probation booking created telemetry event by a probation team`() {
    ...
    with(ProbationBookingCreatedTelemetryEvent(booking, PROBATION_USER)) {
  ...
  @Test
  fun `should raise a probation booking created telemetry event by a delius probation user`() {
    val booking = VideoBooking.newProbationBooking(
      probationTeam = probationTeam(BLACKPOOL_MC_PPOC),
      probationMeetingType = "PSR",
    ...
    with(ProbationBookingCreatedTelemetryEvent(booking, DELIUS_PROBATION_USER)) {
      eventType isEqualTo "BVLS-probation-booking-created"
      properties() containsEntriesExactlyInAnyOrder mapOf(
        "video_booking_id" to "0",
        "created_by" to "probation",
  ...
  fun `should raise a probation booking created telemetry event by a prison`() {
    ...
    with(ProbationBookingCreatedTelemetryEvent(booking, PRISON_USER_BIRMINGHAM)) {
      eventType isEqualTo "BVLS-probation-booking-created"
      properties() containsEntriesExactlyInAnyOrder mapOf(
        "video_booking_id" to "0",
  ...
  fun `should fail to create event if court booking`() {
    val error = assertThrows<IllegalArgumentException> { ProbationBookingCreatedTelemetryEvent(courtBooking(), COURT_USER) }
  ...
  fun `should fail to create event when booking has been amended`() {
    ...
    val error = assertThrows<IllegalArgumentException> { ProbationBookingCreatedTelemetryEvent(amendedBooking, PROBATION_USER) }
