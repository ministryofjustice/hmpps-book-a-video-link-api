...
data class CourtBookingEvent(
  ...
  constructor(vbh: VideoBookingEvent, locations: Map<String, List<Location>>) : this(
    ...
    vbh.mainLocationId.let { key -> locations[vbh.prisonCode]?.singleOrNull { it.key == key }?.description ?: key },
    vbh.preLocationId?.let { key -> locations[vbh.prisonCode]?.singleOrNull { it.key == key }?.description ?: key },
    vbh.postLocationId?.let { key -> locations[vbh.prisonCode]?.singleOrNull { it.key == key }?.description ?: key },
  )
}
