CREATE OR REPLACE VIEW v_booking_contacts AS
    select vlb.video_booking_id, 'COURT' as contact_type, cc.name, cc.position, cc.email, cc.telephone, cc.primary_contact, null as contact_area
    from video_booking vlb, court c, court_contact cc
    where vlb.court_id = c.court_id
    and cc.court_id = c.court_id
    and cc.enabled = true
  UNION
    select vlb.video_booking_id, 'PROBATION' as contact_type, ptc.name, ptc.position, ptc.email, ptc.telephone, ptc.primary_contact, null as contact_area
  ...
  UNION
    select vlb.video_booking_id, 'PRISON' as contact_type, pc.name, pc.position, pc.email, pc.telephone, pc.primary_contact, pc.contact_area
    ...
    and pc.enabled = true
  UNION
    select vlb.video_booking_id, 'THIRD_PARTY' as contact_type, tp.name, tp.organisation as position, tp.email, tp.telephone, tp.primary_contact, null as contact_area
