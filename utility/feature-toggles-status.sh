#!/bin/bash

ENV=$1

BVLS_NAMESPACE="hmpps-book-a-video-link-$ENV"
FEATURE_GREY_RELEASE_PRISONS=$(kubectl -n "$BVLS_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_GREY_RELEASE_PRISONS}' | base64 -d)
FEATURE_PROBATION_ONLY_PRISONS=$(kubectl -n "$BVLS_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_PROBATION_ONLY_PRISONS}' | base64 -d)
FEATURE_COURT_ONLY_PRISONS=$(kubectl -n "$BVLS_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_COURT_ONLY_PRISONS}' | base64 -d)
FEATURE_TEMPORARY_BLOCKING_LOCATIONS=$(kubectl -n "$BVLS_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_TEMPORARY_BLOCKING_LOCATIONS}' | base64 -d)
FEATURE_PUBLIC_PRIVATE_COMMENTS_SYNC=$(kubectl -n "$BVLS_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_PUBLIC_PRIVATE_COMMENTS_SYNC}' | base64 -d)

echo ENVIRONMENT="$ENV"
echo "-----------------------"
echo BVLS - FEATURE_GREY_RELEASE_PRISONS="$FEATURE_GREY_RELEASE_PRISONS"
echo BVLS - FEATURE_PROBATION_ONLY_PRISONS="$FEATURE_PROBATION_ONLY_PRISONS"
echo BVLS - FEATURE_COURT_ONLY_PRISONS="$FEATURE_COURT_ONLY_PRISONS"
echo BVLS - FEATURE_TEMPORARY_BLOCKING_LOCATIONS="$FEATURE_TEMPORARY_BLOCKING_LOCATIONS"
echo BVLS - FEATURE_PUBLIC_PRIVATE_COMMENTS_SYNC="$FEATURE_PUBLIC_PRIVATE_COMMENTS_SYNC"
